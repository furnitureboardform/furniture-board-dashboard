...
export function CornerSystemList({ refreshKey }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    getDocs(query(collection(db, 'cornerSystems'), orderBy('createdAt')))
      .then(snap => {
        setItems(snap.docs.map(d => ({
          docId: d.id,
          ...(d.data() as CornerSystemOption),
        })));
      })
      .catch(console.error)
  ...
  async function handleDelete(docId: string) {
    if (!confirm('Usunąć ten system narożny?')) return;
    setDeletingId(docId);
    try {
      await deleteDoc(doc(db, 'cornerSystems', docId));
  ...
  function handleEditImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    let cancelled = false;
    reader.onload = () => { if (!cancelled) setEditImageBase64(reader.result as string); };
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!editLabel.trim()) { setEditError('Podaj nazwę systemu narożnego.'); return; }
    ...
    if (!editImageBase64) { setEditError('Dodaj zdjęcie systemu narożnego.'); return; }

    setSaving(true);
    try {
      await updateDoc(doc(db, 'cornerSystems', editingItem.docId), {
  ...
  if (loading) return <div className="loading">Ładowanie systemów narożnych…</div>;
  if (items.length === 0) return <div className="empty">Brak systemów narożnych. Dodaj pierwszy!</div>;

  return (
    <>
      <div className="list">
        <h2 className="list-title">Systemy narożne ({items.length})</h2>
      ...
      {editingItem && (
        <div className="modal-overlay" onClick={closeEdit}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2 className="form-title">Edytuj system narożny</h2>
