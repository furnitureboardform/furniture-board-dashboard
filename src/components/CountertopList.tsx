...
import type { CountertopOption } from '../lib/types';
import { FileInput } from './FileInput';

type Item = CountertopOption & { docId: string };

interface Props {
  refreshKey: number;
}

export function CountertopList({ refreshKey }: Props) {
  ...
  async function handleDelete(docId: string) {
    if (!confirm('Usunąć ten blat?')) return;
    setDeletingId(docId);
    try {
      await deleteDoc(doc(db, 'countertops', docId));
  ...
  function openEdit(item: Item) {
    ...
    setEditThickness(String(item.thicknessMm));
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const thicknessNum = parseFloat(editThickness.replace(',', '.'));
    if (!editLabel.trim()) { setEditError('Podaj nazwę blatu.'); return; }
    if (!editBrand.trim()) { setEditError('Podaj nazwę firmy.'); return; }
    if (isNaN(thicknessNum) || thicknessNum <= 0) { setEditError('Podaj prawidłową grubość.'); return; }
    if (isNaN(priceNum) || priceNum <= 0) { setEditError('Podaj prawidłową cenę.'); return; }

    setSaving(true);
    try {
      await updateDoc(doc(db, 'countertops', editingItem.docId), {
      ...
      setItems(prev => prev.map(i =>
        i.docId === editingItem.docId
          ? { ...i, label: editLabel.trim(), brand: editBrand.trim(), thicknessMm: thicknessNum, pricePerSqmPln: priceNum, imageBase64: editImageBase64 }
          : i
      ));
      closeEdit();
    } catch (err) {
  ...
  if (loading) return <div className="loading">Ładowanie blatów…</div>;
  if (items.length === 0) return <div className="empty">Brak blatów. Dodaj pierwszy!</div>;

  return (
    <>
      <div className="list">
        <h2 className="list-title">Blaty kuchenne ({items.length})</h2>
            ...
            <div key={item.docId} className="item-card">
              ...
              <div className="item-body">
                ...
                <div className="item-meta">
                  <span className="badge">{item.thicknessMm} mm</span>
      ...
      {editingItem && (
        <div className="modal-overlay" onClick={closeEdit}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2 className="form-title">Edytuj blat kuchenny</h2>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="field">
                ...
                <input className="field-input" type="text" inputMode="decimal" value={editThickness} onChange={e => setEditThickness(e.target.value)} />
              </div>
              <div className="field">
